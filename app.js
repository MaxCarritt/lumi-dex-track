...
function renderSettings() {
  ...
  MAIN.innerHTML = `<div class="set">
    <div class="r"><div><b>Eden save folder</b><small>${nativeSave ? h(saveSnapshot.status || 'Choose the folder containing SaveData.bin') : 'Automatic save sync is available in the Android APK.'}</small>${nativeSave ? `<small>${h(saveSnapshot.folder || 'In the file picker, open Eden and browse to your game folder.')}</small><small style="color:var(--gold)">Select the folder that directly contains <b>SaveData.bin</b>. For Luminescent Platinum, look for game ID <b>0100000011D90000</b>; do not select the whole Eden, nand, or save folder.</small><pre class="save-path">Android/data/dev.eden.eden_emulator/files/nand/user/save/\n└── 0000000000000000/\n    └── &lt;your profile ID&gt;/\n        └── 0100000011D90000/</pre><small>Read-only · checks every 5 seconds · save catches are locked</small>` : ''}</div>${nativeSave ? '<button class="btn" id="stSaveFolder">Choose folder</button>' : ''}</div>
